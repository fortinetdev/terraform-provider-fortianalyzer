...
func flattenSystemHaPeerIpHb(v interface{}, d *schema.ResourceData, pre string) interface{} {
	return v
}
...
func refreshObjectSystemHaPeer(d *schema.ResourceData, o map[string]interface{}) error {
	...
	if err = d.Set("ip_hb", flattenSystemHaPeerIpHb(o["ip-hb"], d, "ip_hb")); err != nil {
		if vv, ok := fortiAPIPatch(o["ip-hb"], "SystemHaPeer-IpHb"); ok {
			if err = d.Set("ip_hb", vv); err != nil {
				return fmt.Errorf("Error reading ip_hb: %v", err)
			}
		} else {
			return fmt.Errorf("Error reading ip_hb: %v", err)
		}
...
func expandSystemHaPeerIpHb(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	return v, nil
}
