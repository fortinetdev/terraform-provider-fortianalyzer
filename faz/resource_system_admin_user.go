...
func refreshObjectSystemAdminUser(d *schema.ResourceData, o map[string]interface{}) error {
	...
	if isImportTable() {
		if err = d.Set("fazadom", flattenSystemAdminUserAdom(o["adom"], d, "fazadom")); err != nil {
			if vv, ok := fortiAPIPatch(o["adom"], "SystemAdminUser-Adom"); ok {
				if err = d.Set("fazadom", vv); err != nil {
					return fmt.Errorf("Error reading fazadom: %v", err)
				}
			} else {
				return fmt.Errorf("Error reading fazadom: %v", err)
			}
